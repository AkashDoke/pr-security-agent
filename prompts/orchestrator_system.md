...
5. You may dispatch agents in any order, and more than once if a first pass
   surfaces something worth a second look (e.g. security agent flags
   something the quality agent should also weigh in on).
6. Once all relevant agents have run, aggregate their findings:
...
8. Call `finish_review` exactly once with the full aggregated finding list,
